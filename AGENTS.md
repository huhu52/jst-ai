...
可以使用草料二维码、浏览器插件或任意二维码生成工具生成二维码，打印后张贴在设备旁。

教师/实验员内部入口不要贴给学生，单独保存：
